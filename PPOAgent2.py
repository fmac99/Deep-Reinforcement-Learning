...
#        prob_ratio = prob_ratio.mean(dim=2)
    ...
    def Advantage_Estimation(self,values, rewards,gamma,lambduh, tmax):
        ...
        returns=torch.cat(Advantage_List)
        returns = returns.reshape(tmax-1,20)
        advantages = returns -torch.cat(values[0:tmax-1]).reshape(tmax-1,20)
        Normalized_Advan=(advantages -advantages.mean(dim=0))/advantages.std(dim=0)
        return Normalized_Advan, returns
...
#        normalized_advs = (advan -torch.mean(advan,dim=1)[:,np.newaxis] )/torch.std(advan,dim=1)[:,np.newaxis] 
    ...
    def Learn(self,states,value_targets,value_estimate,probs,advs,returns,epsilon,Minb):
        
        value_target =value_targets.reshape(Minb,20)
        value_estimate = value_estimate.reshape(Minb,20)
        value_target=Variable(value_target, requires_grad = True)
        value_estimate=Variable(value_estimate, requires_grad = True)
        Value_Loss = (Variable(returns.reshape(Minb,20),requires_grad=True)-value_target).pow(2).mean()
    ...
    def MakeMinibatch(self,states,values,value_est,probs,advs,returns,tmax,mini_batch):
        batch_dict = {}
